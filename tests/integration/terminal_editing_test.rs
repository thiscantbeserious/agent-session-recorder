...
fn erase_to_end_of_line() {
    ...
    buf.process("Hello World\x1b[1;6H\x1b[K", None);
...
fn erase_line_clears_entire_line() {
    let mut buf = TerminalBuffer::new(80, 24);
    buf.process("Hello World\x1b[1;6H\x1b[2K", None);
...
fn erase_to_end_of_screen() {
    ...
    buf.process("Line1\r\nLine2\r\nLine3", None);
    buf.process("\x1b[2;1H\x1b[0J", None);
...
fn line_wrap_at_width() {
    let mut buf = TerminalBuffer::new(10, 3);
    buf.process("1234567890ABC", None);
...
fn resize_during_playback_sequence() {
    let mut buf = TerminalBuffer::new(80, 24);
    buf.process("Initial content at 80 cols", None);
